package X;

import com.ss.android.ugc.aweme.ecommerce.base.osp.vm.OrderSubmitState;
import kotlin.jvm.internal.o;

/* renamed from: X.AwZ, reason: case insensitive filesystem */
/* loaded from: classes5.dex */
public final class C27859AwZ extends AbstractC65781Prl implements InterfaceC88472Yns<OrderSubmitState, C76800UCe> {
    public static final C27859AwZ LJLIL = new C27859AwZ();

    public C27859AwZ() {
        super(1);
    }

    @Override // X.InterfaceC88472Yns
    public final C76800UCe invoke(OrderSubmitState orderSubmitState) {
        OrderSubmitState it = orderSubmitState;
        o.LJIIIZ(it, "it");
        C27949Ay1.LJIJJ("back", Boolean.valueOf(it.getReachableAny()), 4);
        return C76800UCe.LIZ;
    }
}
