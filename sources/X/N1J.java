package X;

import com.ss.android.ugc.aweme.feed.model.CardStruct;
import com.ss.android.ugc.aweme.log.AdWebLogModel;
import kotlin.jvm.internal.o;

/* loaded from: classes11.dex */
public final class N1J extends AbstractC65781Prl implements InterfaceC88472Yns<M95, String> {
    public static final N1J LJLIL = new N1J();

    public N1J() {
        super(1);
    }

    @Override // X.InterfaceC88472Yns
    public final String invoke(M95 m95) {
        M95 $receiver = m95;
        o.LJIIIZ($receiver, "$this$$receiver");
        Object LIZ = $receiver.LIZ(AdWebLogModel.class);
        if (LIZ != null) {
            String sessionId = ((AdWebLogModel) LIZ).getSessionId();
            if (sessionId == null) {
                return CardStruct.IStatusCode.DEFAULT;
            }
            return sessionId;
        }
        throw new C58623Mzb(AdWebLogModel.class);
    }
}
