...
public final class TRE implements U62 {
    ...
    public TRE(C74723TUh c74723TUh) {
    ...
    public final void LJIIJJI(InterfaceC75441TjB session, LeaveMessage leaveMessage) {
        ...
        Long userId;
        o.LJIIIZ(session, "session");
        LinkUser leftUser = leaveMessage.getLeftUser();
        if (leftUser != null && (userId = leftUser.getUserId()) != null) {
            j = userId.longValue();
        } else {
            j = 0;
        }
        ((HashSet) C74838TYs.LJ().LJIL).remove(leftUser.getUserId());
        Iterator<AbstractC74724TUi> it = this.LJLIL.LIZIZ().iterator();
        while (it.hasNext()) {
            AbstractC74724TUi next = it.next();
            if ((j > 0 && next.LJFF() == j) || TextUtils.equals(next.LIZLLL(), leftUser.getLinkMicId())) {
                it.remove();
            }
        }
    }
