...
public class Pose {
    ...
    public Pose inverse() {
        Quaternion g = this.quaternion.g();
        Quaternion.j(g, this.translation, 0, r3, 0);
        float[] fArr = {-fArr[0], -fArr[1], -fArr[2]};
        return new Pose(fArr, g);
    }
    ...
    public Pose compose(Pose pose) {
        Quaternion.j(this.quaternion, pose.translation, 0, r4, 0);
        float f = r4[0];
        float[] fArr = this.translation;
        float[] fArr2 = {f + fArr[0], fArr2[1] + fArr[1], fArr2[2] + fArr[2]};
        return new Pose(fArr2, this.quaternion.h(pose.quaternion));
    }
    ...
    public static Pose makeInterpolated(Pose pose, Pose pose2, float f) {
        ...
        return new Pose(fArr, Quaternion.i(pose.quaternion, pose2.quaternion, f));
    }
    ...
    public void getTransformedAxis(int i, float f, float[] fArr, int i2) {
        ...
        fArr2[i] = f;
        Quaternion.j(quaternion, fArr2, 0, fArr, i2);
    }

    public void rotateVector(float[] fArr, int i, float[] fArr2, int i2) {
        Quaternion.j(this.quaternion, fArr, i, fArr2, i2);
    }
